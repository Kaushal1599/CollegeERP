...
from Admin_module.models import Notices
...
def Teacher_Dashboard(request):
    ...
    Academy_Info = Teacher_Academy_Info.objects.all().filter(teacher_ID_id=username)

    Class = None
    for teacher in Academy_Info:
    ...
    Details = Notices.objects.filter(
        User='All').order_by('-Notice_Date') | Notices.objects.filter(User='Teacher').order_by('-Notice_Date')

    return render(request, 'Teacher/Dashboard.html', {'Basic_Info': Basic_Info, 'class_teacher': class_teacher, 'Details': Details})
...
def Student_Dashboard(request):
    ...
    Details = Notices.objects.filter(User='All').order_by(
        '-Notice_Date') | Notices.objects.filter(User='Student').order_by('-Notice_Date')

    username = request.user.username
    Basic_Info = Student_General_Info.objects.get(Student_ID=username)
    return render(request, 'Student/Dashboard.html', {'Basic_Info': Basic_Info, 'Details': Details})
